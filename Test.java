...
public class Test
{
	...
	static int Duration = 3000; // seconds;  Program will exit after Duration of seconds.

	static int ReferenceSize = 1024;  // each reference object size;
  static int CountDownSize = 1000 * 1000;
  static int EachRemoveSize = 1000 *100; // remove # of elements each time.

	static int PermanentSize = 1000*100; // These will not be removed
...
class LoadThread extends Thread {
	...
	public void run() {
		...
		AbstractQueue<String> pq = new ArrayBlockingQueue<String>(Test.CountDownSize);
    ...
    // Permanent objects;
		for (int i=0; i< Test.PermanentSize; i++) {
